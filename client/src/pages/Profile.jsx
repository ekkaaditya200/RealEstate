...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(updateUserStart(error.message));
    }
  }
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <img
          ...
          src={formData.avatar || currentUser.avatar} alt="profile" className="rounded-full h-24 w-24 object-cover cursor-pointer self-center"></img>
      ...
      {userListing.length > 0 ? (
          ...
          <h1 className="text-center my-7 text-2xl">Your Listing</h1>
          {
            userListing.map((listing, index) => (
              <div key={index} className="border rounded-lg p-3 flex justify-between items-center gap-4">
                <Link to={`/listing/${listing._id}`}><img src={listing.imageUrls[0]} alt="listing/image" className="h-16 w-16 object-contain"></img></Link>
        ...
        </div>) : (<div>No Listing found. First create your listing </div>)
      }
    </div>
  )
}
